...
import type { AssetType, HistoryEntry, Recommendation } from '../types';

interface HistoryPanelProps {
  isOpen: boolean;
  onClose: () => void;
  history: HistoryEntry[];
  onSelect: (ticker: string, assetType: AssetType) => void;
  onClear: () => void;
}
...
const HistoryPanel: React.FC<HistoryPanelProps> = ({
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-[var(--color-background)] shadow-2xl z-50 transform transition-transform duration-300 ease-in-out border-l border-[var(--color-border)] ${
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex justify-between items-center p-4 border-b border-[var(--color-border)]">
            <h2 id="history-panel-title" className="text-xl font-bold text-white">
              Analysis History
            </h2>
            <button
              onClick={onClose}
          ...
          <div className="flex-grow overflow-y-auto p-4">
            {history.length > 0 ? (
              <ul className="space-y-3">
                {history.map((entry, index) => (
                  <li key={`${entry.ticker}-${entry.valuationDate}-${index}`}>
                    <button
                      onClick={() => onSelect(entry.ticker, entry.assetType ?? 'stock')}
                    ...
                    >
                      <div className="flex justify-between items-center mb-1">
                        <p className="font-bold text-lg text-white">
                          {entry.ticker}
                          {entry.assetType === 'etf' && (
                            <span className="ml-2 text-xs font-normal text-cyan-400 border border-cyan-400/30 rounded px-1">
                              ETF
                            </span>
                          )}
                          {' — '}
                          <span className="font-normal text-gray-300">{entry.companyName}</span>
                        </p>
                        <span
                          className={`px-2 py-1 text-xs font-semibold rounded-full border ${
                            recommendationStyles[entry.recommendation]
                          }`}
                        >
                      ...
                      {entry.assetType !== 'etf' && entry.intrinsicValue > 0 && (
                        <p className="text-sm text-gray-500 mt-1">
                          Graham Number: ${entry.intrinsicValue.toFixed(2)}
                        </p>
                      )}
                    </button>
                  </li>
              ...
              <div className="flex flex-col items-center justify-center h-full text-center text-gray-500">
                ...
                <p>Perform an analysis to see it here.</p>
              </div>
            )}
          </div>
